...
class xmlUpdate(ConfigListScreen, Screen):
	def __init__(self, session):
		...
		self.url = "https://raw.githubusercontent.com/OpenLD/openld-tuxbox-common/master/src/%s.xml"
		self.source = ConfigSelection(default="OE-OpenLD", choices=[("OE-OpenLD", _("OE-OpenLD"))])
		self.DVBtype = ConfigSelection(default="satellites", choices=[("satellites", _("satellite")), ("cables", _("cable")), ("terrestrial", _("terrestrial")), ("atsc", _("atsc"))])
	...
	def createSetup(self):
		...
		self.list.append(getConfigListEntry(_("Fetch"), self.DVBtype, _('File being updated, i.e. satellites.xml, cables.xml, terrestrial.xml or atsc.xml.')))
	...
	def validXML(self, XMLdata): # Looks for closing documentElement, i.e. </satellites>, </cables>, or </locations>
		return self.DVBtype.value in ('satellites', 'cables', 'atsc') and ("</%s>" % self.DVBtype.value) in XMLdata or self.DVBtype.value == "terrestrial" and "</locations>" in XMLdata
